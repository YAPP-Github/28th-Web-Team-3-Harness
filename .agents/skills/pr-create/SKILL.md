...
- `$1`: PR을 올릴 **브랜치**(head). 생략 시 현재 브랜치 사용.
...
3. **behind 확인**: `git log HEAD..origin/main --oneline` — 출력이 있으면 브랜치가 `origin/main`보다
   뒤처진 것. 사용자에게 알리고 rebase/merge 후 재개할지 확인. 이 확인을 통과해야
   Step 7 체크리스트의 "최신 코드를 pull 받았나요?" 항목을 `[x]`로 채울 수 있다.
4. 변경 파일 표시: `git diff --stat --merge-base origin/main HEAD`.
5. 베이스보다 앞선 커밋이 없으면 중단 — PR할 게 없음.
...
5. **시크릿 검증**: diff에서 **값이 할당된 형태**의 시크릿 패턴을 검사한다 — 단순 키워드 언급은
   매칭하지 않는다(이 스킬 문서처럼 패턴을 설명만 하는 라인이 오탐되는 것을 방지):

```bash
git diff --merge-base origin/main HEAD | grep -inE \
  '(api[_-]?key|secret|password|token)["'"'"']?\s*[:=]\s*["'"'"']?[A-Za-z0-9_/+.-]{8,}|Bearer [A-Za-z0-9._~+/-]{16,}|-----BEGIN [A-Z ]*PRIVATE KEY'
```

   매칭되면 해당 라인을 사용자에게 보여주고 중단. 사용자가 오탐(예시·플레이스홀더)이라고
   확인하면 그 라인만 예외 처리하고 통과 — 통과 사유를 완료 보고에 남긴다.
...
2. 리뷰를 디스패치한다:
   ...
   - **Codex 교차 리뷰** — openai-codex 플러그인이 설치된 경우에만. 리뷰 전용 companion script를
     실행하되, **버전 디렉터리를 먼저 하나로 해석**하고(glob을 node 인자에 직접 넘기면 여러 버전이
     전부 확장돼 첫 경로만 실행됨), 베이스를 명시한다:

```bash
SCRIPT=$(ls -d ~/.claude/plugins/cache/openai-codex/codex/*/ | sort -V | tail -1)scripts/codex-companion.mjs
node "$SCRIPT" review --base origin/main --wait
```

     (`--wait` 필수: 게이트라서 동기 완료 필요. `--base origin/main`: 로컬 `main`이 낡았을 때
     리뷰 범위가 어긋나는 것을 방지.)
     플러그인이나 Codex CLI가 없으면 생략하고 "Codex 교차 리뷰 생략됨"만 알림 — 실패로 취급하지 않는다.
3. 리뷰 결과(판정 + 발견 사항) 모두 사용자에게 표시. 두 리뷰가 실행됐으면 최종 판정은 더 나쁜 쪽 채택
...
1. **assignee = `ME`**(Step 0, 스킬을 실행한 사람).
2. 리뷰어 = 위 4명 중 `ME`를 제외한 나머지 3명. `ME`가 표에 없으면 4명 전원을 리뷰어로 하고,
   표에 없는 실행자임을 사용자에게 알릴 것.
3. 제외 후 리뷰어가 0명이 되는 경우는 없다(표가 4명이므로 최소 3명 보장).
