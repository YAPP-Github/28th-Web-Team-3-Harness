...
- [미션 생성 API 필드 매핑](../domain/api-field-mappings/mission-generation.md)은 고정 FE가 사용하는 생성 작업 API와 BE에 별도로 남은 동기 후보 조회 계약을 분리해 기록한다. 동기 후보 응답에는 확정·저장 식별자가 없어 현재 사용자 생성 플로우로 사용하지 않는다.
